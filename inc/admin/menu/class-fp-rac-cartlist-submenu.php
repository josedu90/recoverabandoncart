...
if (!class_exists('FP_RAC_Cartlist_Table')) {
    ...
    class FP_RAC_Cartlist_Table {
        ...
        public static function fp_rac_display_cartlist_table_data($column, $postid) {
            ...
            switch ($column) {
                ...
                case 'rac_cart_email_id':
                    ...
                    break;
                case 'rac_capture_by':
                    echo self::display_abandon_email_capture_by($cart_list);
                    break;
        ...
        public static function fp_rac_add_cartlist_filter_option($post_type) {
            if ($post_type == 'raccartlist') {
                ...
                ?><select name="fprac_cartlist_tag">
                    ...
                    <option value = 'users' <?php selected($selected_value, 'users'); ?> ><?php echo __('Users', 'recoverabandoncart') . ' (' . $users_count . ')'; ?></option>
                    <option value = 'guest' <?php selected($selected_value, 'guest'); ?> ><?php echo __('Guests', 'recoverabandoncart') . ' (' . $guests_count . ')'; ?></option>
                </select>
                <?php
        ...
        private static function get_cartlist_ids($guest = false){
            global $wpdb;
            $operator = ($guest) ? '=' : '!=';
            $post_status = implode("','", array('rac-cart-new', 'rac-cart-abandon', 'rac-cart-recovered'));
            $user_guests_list = $wpdb->get_col($wpdb->prepare("SELECT p.ID FROM  {$wpdb->posts} AS p "
                    . "WHERE p.post_type= %s AND "
                    . "p.post_status IN ('$post_status') "
                    . "AND p.post_author $operator %d ", 'raccartlist', '0'));

            return $user_guests_list;
        }
        ...
        public static function fp_rac_subscribe_emails_filter($where, $wp_query) {
            ...
            if (!empty($_REQUEST['fprac_cartlist_tag'])) {
                ...
                } elseif (($_REQUEST['fprac_cartlist_tag'] == 'subscribe')) {
                    $post_ids = self::fp_rac_subcribe_email_count();
                } elseif (($_REQUEST['fprac_cartlist_tag'] == 'users')) {
                    $post_ids = self::get_cartlist_ids();
                } elseif (($_REQUEST['fprac_cartlist_tag'] == 'guest')){
        ...
        public static function fp_rac_post_orderby_functionality($order_by, $wp_query) {
            ...
            if (isset($_REQUEST['post_type']) && $_REQUEST['post_type'] == 'raccartlist') {
                ...
                if (!isset($_REQUEST['order']) && !isset($_REQUEST['orderby'])) {
                    ...
                    } elseif($_REQUEST['orderby'] == "post_status") {
                          $post_status = $_REQUEST['orderby'];
                          $order_by = "{$wpdb->posts}.post_status ". $_REQUEST['order'];
                    }
                }
            }
            return $order_by;
        ...
        public static function fp_rac_cartlist_export_csv() {
            ...
            if (rac_check_is_array($posts)) {
                foreach ($posts as $post) {
                    ...
                    if ( $obj_cart_lists->user_id == '0') {
                      $details = maybe_unserialize($obj_cart_lists->cart_details);
                      $obj_cart_lists->phone_no = isset($details['visitor_phone']) ? $details['visitor_phone'] : '-';
                    }
                    elseif ( $obj_cart_lists->user_id == 'old_order') {
            ...
            $row_heading = array('id', 'cart_details', 'user_id', 'email_id', 'phone_number', 'cart_abandon_time', 'cart_status', 'mail_template_id', 'ip_address', 'link_status', 'sending_status', 'wpml_lang', 'placed_order', 'completed');
        ...
        public static function fp_rac_display_cart_list_details_column($each_list) {
            ...
            if ($product_details == 'no data') {
                ...
                echo '<p style="text-decoration: none;" >' . fp_rac_cart_details($each_list) . '</p>';
            }
        ...
        public static function fp_rac_display_cart_list_user_details_column($each_list) {
            ...
            if (is_object($user_info)) {
                ...
                if (is_array($cart_array)) {
                    ...
                    $guest_first_last =' / ' . fp_rac_get_order_obj_data($cart_array, "billing_first_name") . ' ' . fp_rac_get_order_obj_data($cart_array, "billing_last_name");
                }
            ...
            if ('0' == $each_list->user_id) {
                ...
                if (is_object($details)) {
                    ?> <div class="rac_tool_info"><p class="rac_edit_option" data-id="<?php echo $each_list->id; ?>" >
                        ...
                        echo $guest_email; // Order Object. Works for both old order and rac captured order

                        ?></p><div class="tooltip"><?php _e("Double Click to Change an Editable", "recoverabandoncart"); ?></div></div><?php
                            echo '</br>&nbsp' . fp_rac_get_order_obj_data($details, 'billing_phone');
                ...
                private static function display_abandon_email_capture_by($each_list) {
                    if ($each_list->capture_by == 1)
                      _e('Cart Page', 'recoverabandoncart');
                    elseif ($each_list->capture_by == 2)
                      _e('Checkout Page', 'recoverabandoncart');
                    elseif ($each_list->capture_by == 3)
                      _e('Order Page', 'recoverabandoncart');
                    elseif ($each_list->capture_by == 4)
                      _e('Popup Page', 'recoverabandoncart');
                    else
                      _e('its available on latest version', 'recoverabandoncart');
                }
                ...
                private static function fp_rac_display_cart_list_status_column($each_list) {
                    ...
                    if ($post_status == 'trash') {
                        ...
                        if($post_status_method == 1)
                            echo $post_name . '<br>' . '(' . __('Manually Recovered', 'recoverabandoncart') . ')';
                        else
                            echo $post_name;
                    }
        ...
        private static function fp_rac_display_cart_list_email_template_column($each_list) {
            ?><a href="#" class="button rac_cartlist_email_status_info rac_mail_status_btn" data-rac_cart_list_id="<?php echo $each_list->id; ?>">
              <img src="<?php echo RAC_PLUGIN_URL . '/assets/images/view.png'; ?>">
            </a><?php
        }
        ...
        private static function fp_rac_display_cart_list_recovered_orderid_column($each_list) {
            ...
            if($post_status == 'rac-cart-recovered' && empty($each_list->placed_order)){

              ?><a href="#" class="button rac_cartlist_manual_recover_popup_link" data-rac_cart_list_id="<?php echo $each_list->id; ?>"><?php echo __("Enter Order ID", "recoverabandoncart"); ?></a><?php
            } else{
